...
## Reproducibility amendment: deterministic tokenizer

Added 2026-08-31 after exploratory outcome inspection and before any confirmatory run. NVIDIA's LIBERO evaluator sets both `PolicyEvalConfig.deterministic=True` and the `DETERMINISTIC=True` environment variable. The latter fixes VAE encoding noise in the public policy tokenizer. Our initial direct-inference scripts set only the config field. All artifacts produced before this amendment are therefore exploratory, and all confirmatory branch construction, controls, and interventions must be regenerated with the environment switch enabled. The analysis choices above are unchanged. New run summaries explicitly record deterministic-tokenizer status, and paired GPU collections must agree bitwise before outcome analysis.
