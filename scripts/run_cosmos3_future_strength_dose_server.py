#!/usr/bin/env python3
"""Serve the prospective Cosmos 3 future-strength dose-response study.

The default request path remains compatible with RoboLab's OpenPI client. A
research client can additionally register deterministic native predictions and
request self, zero-site, or five-level recipient-to-donor future clamps after
an exact transformed-observation fingerprint check. Other intervention modes
and attention instrumentation are rejected by the public request interface.
"""
...
from imagined_future.cosmos3_dose_response import ALPHAS, validate_alpha
...
class ResearchPolicyService:
        ...
        *,
        ...
        if mode in {"self", "none"}:
            if alpha is not None:
                raise ValueError(f"{mode} mode does not accept research_alpha")
            target = recipient.target
            target_source = "recipient"
        elif mode == "donor":
            if alpha is not None:
                raise ValueError("donor mode does not accept research_alpha")
            target = donor.target
            target_source = "donor"
        elif mode == "gaussian":
            if alpha is not None:
                raise ValueError("gaussian mode does not accept research_alpha")
            target = gaussian_target_on_mask(recipient.target, donor.target, mask, seed=gaussian_seed)
            target_source = "gaussian_geometry"
        elif mode == "dose":
            if alpha is None:
                raise ValueError("dose mode requires research_alpha")
            alpha = validate_alpha(alpha)
            # Endpoint targets are direct registered tensors; never obtain them
            # through floating-point cancellation. Interior targets interpolate
            # only future frames in the registered tensor's native dtype.
            if alpha == 0.0:
                target = recipient.target
            elif alpha == 1.0:
                target = donor.target
                ...
                target[mask] = recipient.target[mask] + alpha * (
                    donor.target[mask] - recipient.target[mask]
                )
        ...
        audit = {
            ...
            "target_coordinate_count": int(target.numel()),
            "target_finite_coordinate_count": int(
                torch.isfinite(target.float()).sum().item()
            ),
        ...
        if mode == "dose":
            ...
            expected_future = (
                ...
                if alpha == 0.0
                else donor.target[mask]
                if alpha == 1.0
                else recipient.target[mask]
                + alpha * (donor.target[mask] - recipient.target[mask])
            )
            nonfuture_mask = ~mask
            current_frame_mask = temporal_mask(
                donor.vision_shape, (0,), device=torch.device("cpu")
            )
            audit.update(
                {
                    "alpha": float(alpha),
                    "alpha_grid": np.asarray(ALPHAS, dtype=np.float32),
                    "interpolation_formula": "F_A + alpha * (F_B - F_A)",
                    "interpolation_formula_max_abs_error": float(
                        (target[mask] - expected_future).abs().max()
                    ),
                    "interpolated_future_hash": tensor_digest(target[mask]),
                    "recipient_future_mask_hash": tensor_digest(recipient.target[mask]),
                    "donor_future_mask_hash": tensor_digest(donor.target[mask]),
                    "current_frame_hash": tensor_digest(target[current_frame_mask]),
                    "recipient_current_frame_hash": tensor_digest(
                        recipient.target[current_frame_mask]
                    ),
                    "future_mask_hash": tensor_digest(mask),
                    "nonfuture_recipient_target_max_abs_error": float(
                        (target[nonfuture_mask] - recipient.target[nonfuture_mask])
                        .abs()
                        .max()
                    ),
                    "alpha_zero_recipient_future_max_abs_error": (
                        float((target[mask] - recipient.target[mask]).abs().max())
                        if alpha == 0.0
                        else None
                    ),
                    "alpha_one_donor_future_max_abs_error": (
                        float((target[mask] - donor.target[mask]).abs().max())
                        if alpha == 1.0
                        else None
                    ),
                    "alpha_zero_target_hash_matches_recipient": (
                        tensor_digest(target) == tensor_digest(recipient.target)
                        if alpha == 0.0
                        else None
                    ),
                    "alpha_one_target_hash_matches_donor": (
                        tensor_digest(target) == tensor_digest(donor.target)
                        if alpha == 1.0
                        else None
                    ),
                }
    ...
    def infer(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        if mode not in {"native", "none", "self", "dose"}:
            raise ValueError(f"dedicated dose server rejects research_mode {mode!r}")
        if any(str(key).startswith("research_attention_") for key in obs):
            raise ValueError("dedicated dose server rejects attention interventions")
        if mode == "dose" and "research_alpha" not in obs:
            raise ValueError("dose mode requires research_alpha")
        if mode != "dose" and "research_alpha" in obs:
            raise ValueError(f"research_alpha is invalid for {mode!r} mode")
        ...
        with (
            ...
            if mode == "native":
                ...
                if bool(obs.get("research_return_video", False)):
                    ...
                    outputs["video"] = video.cpu().numpy()
            elif mode in {"none", "self", "donor", "gaussian", "dose"}:
                ...
                samples, audit = self._intervene(
                    ...
                    alpha=(
                        validate_alpha(float(obs["research_alpha"]))
                        if mode == "dose" and "research_alpha" in obs
                        else None
                    ),
                )
...
def main() -> None:
    ...
    if args.attention_instrumentation:
        raise ValueError("the dedicated dose server forbids attention instrumentation")
