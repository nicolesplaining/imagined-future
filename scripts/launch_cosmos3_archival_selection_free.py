#!/usr/bin/env python3
"""Resume-safe deterministic shard launcher for the frozen archival study."""
...
def validate_completed(
    ...
    *,
    ...
    if len(payload.get("future_source_retrieval_rows", [])) != 16:
        raise RuntimeError(f"existing output lacks the frozen 4x4 retrieval grid: {path}")
    if len(payload.get("donor_rows", [])) != 12:
        raise RuntimeError(f"existing output lacks the frozen donor subset: {path}")
    if payload.get("request_count") != 56:
        raise RuntimeError(f"existing output lacks exactly 56 frozen requests: {path}")
    coordinate_errors = payload.get("action_coordinate_errors", {})
    if len(coordinate_errors) != 48 or any(
        pair != {"input": 0.0, "output": 0.0}
        for pair in coordinate_errors.values()
    ):
        raise RuntimeError(f"existing output lacks 48 exact action-coordinate audits: {path}")
    ...
    if (
        ...
        raise RuntimeError(f"existing output lacks 44 paired finite residuals: {path}")
    none_controls = payload.get("none_controls", [])
    if len(none_controls) != 4 or any(
        row.get("action_maximum_error_vs_native") != 0.0
    ...
    if len(site_audits) != 48:
        raise RuntimeError(f"existing output lacks 48 intervention-site audits: {path}")
    active_sites = sum(int(row.get("active_site_count", -1)) for row in site_audits.values())
    if active_sites != 176 or payload.get("active_intervention_site_count") != 176:
        raise RuntimeError(f"existing output lacks exactly 176 active sites: {path}")
    ...
    if set(payload.get("native_repeat_action_maximum_error", {}).values()) != {0.0}:
        raise RuntimeError(f"existing output failed native action replay: {path}")
    if set(
        payload.get("native_deterministic_metadata_replay_exact", {}).values()
    ) != {True}:
        raise RuntimeError(f"existing output failed native metadata replay: {path}")
...
def main() -> None:
    ...
    if manifest.get("admission") != "frozen_archival_selection_free_action_level_evaluation":
        raise ValueError("launcher refuses a non-evaluation manifest")
    if len(manifest.get("states", [])) != 90:
        raise ValueError("frozen archival evaluation must contain exactly 90 states")
    ...
    checkpoint_manifest_audit = Path(
        manifest["runtime"]["checkpoint_content_manifest_audit_path"]
    )
    if sha256(checkpoint_manifest_audit) != manifest["runtime"][
        "checkpoint_content_manifest_audit_sha256"
    ]:
        raise ValueError("checkpoint content-manifest audit differs from frozen manifest")
    ...
    if args.output_root.resolve() != Path(
        ...
        raise ValueError("requested output root differs from the fresh v7 root")
    selected = manifest["states"][args.shard_index :: args.shard_count]
    if len(selected) != 90:
        raise RuntimeError("the sequential general-server shard must assign all 90 states")
